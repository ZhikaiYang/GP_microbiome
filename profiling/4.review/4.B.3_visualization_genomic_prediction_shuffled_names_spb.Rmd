...
fq <- list.files(path="largedata/review/output_shuffled_sample_names", pattern="prediction_accuracy_seed.*_shuffled_quadrant_yes.*_3626asvs_spb", full.names = T)
...
fwrite(out[,-7], "largedata/review/pred_seeds_vis_shuffled_sample_names_3626asvs_spb_from_count_nlogrel.txt", sep = "\t", quote = F, row.names = F, col.names = T)
...
f <- list.files(path="largedata/review/output_shuffled_sample_names", pattern="prediction_accuracy_seed.*_shuffled_quadrant_yes.*_3626asvs_spb", full.names = T)
...
out$trait  <- gsub(".*trait_|_date.*", "", out$file)
#out$date = gsub(".*date_|_shuffled.*", "", out$file)
out$shuffled  <- gsub(".*shuffled_|_3626asvs_spb.*", "", out$file)

id_r = which((out$method == "rrBLUP_PC_microbiome") | (out$method == "rrBLUP"))
out = out[-id_r,]


fwrite(out[,-7], "largedata/review/pred_seeds_vis_shuffled_quandrant_yes_3626asvs_spb.txt", sep = "\t", quote = F, row.names = F, col.names = T)
...
my_comparisons <- list(c("SNP", "SNP + ASV"), c("SNP", "SNP + shuffled ASV by sample names"),  c("SNP + ASV", "SNP + shuffled ASV by sample names within quadrant"),   c("SNP + shuffled ASV by sample names", "SNP + shuffled ASV by sample names within quadrant"))
