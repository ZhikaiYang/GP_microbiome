...
```{r}
library(data.table)
f <- list.files(path="largedata/review/output_154Functional_Tax_Groupes", pattern="prediction_accuracy_seed.*_nitrogen_", full.names = T)
...
id_pc = which(out$method == "rrBLUP_PC_microbiome")
out = out[-id_pc,]
fwrite(out[,-7], "largedata/review/pred_seeds_nitrogen_vis_shuffled_names_154tax.txt", sep = "\t", quote = F, row.names = F, col.names = T)
...
```{r}
pred_all_nitrogen = fread("largedata/review/pred_seeds_nitrogen_vis_shuffled_names_154tax.txt", data.table = F)
