...
f <- list.files(path="largedata/review/output_shuffled_sample_names", pattern="prediction_accuracy_seed", full.names = T)
...
out = out[-id_pc,]
fwrite(out[,-7], "largedata/review/pred_seeds_vis_shuffled_sample_names.txt", sep = "\t", quote = F, row.names = F, col.names = T)
...
pred_all_compare = pred_all_compare[-(which(pred_all_compare$trait == "Canopy_Coverage")),]
...
pred_shuffled_sample_names = fread("largedata/review/pred_seeds_vis_shuffled_sample_names.txt", data.table = F)

pred_shuffled_sample_names_avg =  as.data.frame( pred_shuffled_sample_names %>% group_by(trait, date, method, seed) %>% summarise_all(mean))

pred_shuffled_sample_names_avg = pred_shuffled_sample_names_avg[-(which(pred_shuffled_sample_names_avg$method == "rrBLUP")),]

pred_shuffled_sample_names_avg$method[which(pred_shuffled_sample_names_avg$method == "rrBLUP_geno_microbiome")] = "SNP + shuffled ASV by sample names"

pred_all_compare = rbind(pred_all_compare, pred_shuffled_sample_names_avg)

pred_all_compare$date <- factor(pred_all_compare$date, levels = c( "Aug12", "Aug22", "Sept5"), labels =c( "Aug-12", "Aug-22", "Sep-5"))

pred_all_compare$method <- factor(pred_all_compare$method, levels = c( "rrBLUP", "rrBLUP_geno_microbiome", "rrBLUP_geno_shuffled_microbiome", "SNP + shuffled ASV by sample names"), labels =c( "SNP", "SNP + ASV", "SNP + indepently shuffled ASV", "SNP + shuffled ASV by sample names"))
...
my_comparisons <- list(c("SNP", "SNP + ASV"),c("SNP", "SNP + indepently shuffled ASV"), c("SNP", "SNP + shuffled ASV by sample names"), c("SNP + indepently shuffled ASV", "SNP + shuffled ASV by sample names"))
