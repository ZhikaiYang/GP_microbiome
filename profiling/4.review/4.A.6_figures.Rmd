...
pdf("largedata/review/figures/p_vari_shuffled_spb.pdf", width = 13, height= 6)
p_vari_shuffled
dev.off()
...
p_vari_shuffled_quadrant = ggarrange(p_vari_shuffled_aug12, p_vari_shuffled_aug22, p_vari_shuffled_sep5, nrow = 3, ncol = 1)

p_vari_shuffled_quadrant


pdf("largedata/review/figures/p_vari_shuffled_quadrant_spb.pdf", width = 8, height= 13)
p_vari_shuffled_quadrant
dev.off()
...
p_vari_shuffled_hn_aug12 = ggboxplot(pred_all_nitrogen_avg[which((pred_all_nitrogen_avg$trait == vis[i])& (pred_all_nitrogen_avg$nitrogen == "HN") & (pred_all_nitrogen_avg$date == "Aug-12")),], x = "method", y = "accuracy", color = "method", palette = "jco",add = "jitter") +
    stat_compare_means(label = "p.signif", method = "t.test", aes(group = method), paired = F)+
    xlab("") +
    ylab("Prediction Accuracy ") +
    ggtitle(paste(vis[i], "Aug-12", sep = "_")) +
    theme_classic() +
...
compare_means(accuracy ~ method, pred_yieldc_avg[which( (pred_yieldc_avg$trait == yieldc[i])),], method = "t.test", paired = T)
...
p_vari_shuffled_hn = ggboxplot(pred_all_nitrogen_avg[which((pred_all_nitrogen_avg$trait == vis[i]) & (pred_all_nitrogen_avg$nitrogen == "HN")),], x = "method", y = "accuracy", color = "method", palette = "jco",add = "jitter") +
    stat_compare_means(comparisons = my_comparisons, label = "p.signif", method = "t.test", aes(group = method), paired = T)+
    xlab("") +
    ylab("Prediction Accuracy ") +
