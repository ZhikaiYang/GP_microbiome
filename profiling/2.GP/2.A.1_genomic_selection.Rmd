...
colnames(Z) = colnames(geno2)
Z = as.data.frame(Z)
Z= cbind(genos$genotype, Z)
colnames(Z)[1]= "genotype"
Z$genotype = as.character(Z$genotype)
fwrite(Z,"largedata/hmp321_282_agpv4_maf005_miss02_pruned_s50k_imputed_matrix.txt",sep="\t",col.names = T,row.names = F,quote=F)
...
pdf("largedata/micrbiome.pdf", width=10, height=8)
ggpairs(microbiomes[,sort(sample(152:ncol(microbiomes),6))],title = "Sampled sub-groups of microbiome")
dev.off()
